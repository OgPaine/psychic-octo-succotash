...
useEffect(() => {
    if (state.gameActive) {
        const interval = setInterval(() => {
            ...
            const newWidth = (newTime / initialState.timer) * 100;
            setProgressWidth(newWidth);
            if (newTime <= 0) {
                clearInterval(interval);
                setProgressWidth(0); // Ensure it hits 0 when the timer ends
                setTimeout(() => {
                    gameOver();
                    setMessage(`Time's up! The correct PIN was ${state.pin.join("")}`);
                }, 1000); // Delay showing the message to allow the progress bar animation to finish
            }
        }, 1000);
        return () => clearInterval(interval);
    } else {
        // Stop the progress bar when the game is not active
        setProgressWidth(0);
    }
    ...
    const handleKeyDown = (event, index) => {
        if (event.key === "Enter") {
            ...
            if (isCorrect) {
                ...
                // Optionally, clear any previous message or leave it as the last state
                // setMessage("");  // Uncomment this line if you want to clear the message when the guess is wrong
            }
    ...
    const startGame = () => {
        ...
        setProgressWidth(100); // Reset the progress bar to full
        dispatch({
            type: "start_game",
  ...
  <div className="flex items-center justify-center h-screen bg-[#04131C]">
    <div className="bg-[#082030] w-[500px] p-0 pb-0 rounded text-center relative box-border">
      ...
      <div className="bg-[#062d40] rounded p-6 mb-0 mx-6">
    ...
    />
      ...
      <div className="w-full bg-gray-700 h-2 relative mt-4">
          <div className="bg-[#fc4207] h-2 transition-all duration-1000 ease-linear" style={{ width: `${progressWidth}%` }}></div>
      </div>
    </div>
  </div>
);
